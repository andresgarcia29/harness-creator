...
| **Carril** (lane) | El tamaño de proceso que se le asigna a una tarea según su blast radius: `quick`, `express`, `standard` o `full`. | Es el mecanismo concreto de lo anterior. En `express` una tarea usa 2 sesiones de modelo en vez de 6, y en `quick` ni siquiera se delibera, siempre con los **mismos** gates. |
...
4. [El diagrama maestro: qué pasa cuando corres `/smart`](#el-diagrama-maestro-qué-pasa-cuando-corres-smart)
...
```
/smart COR-123                                  # un ticket de Linear
/smart "agrega rate limiting por tenant al gateway, 100 req/min"   # o un prompt literal
...
│   ├── commands/             ← /smart (todo el pipeline) + /quick (lo trivial)
...
│   ├── orchestrator-watch.sh ← vigila la sesión que orquesta: relanza lo varado y ejecuta el relevo de fase
...
│   ├── deploy-watch.sh       ← vigila Actions→Kargo→ArgoCD→rollout→smoke; rollback seguro
...
└── tasks/<id>/               ← estado por tarea: task.md, enrichment.md, assumptions.md,
                                 state.json (fase + carril + sesión, lo mueve harness-policy.py),
                                 agents.json (ids para continuar agentes), handoff.json
                                 (relevo de fase), plan.md, veredictos, evidencia, logs
```

---

## El diagrama maestro: qué pasa cuando corres `/smart`

Primero la **espina dorsal**: las tres entradas, dónde despierta cada agente, qué lo bloquea, y el hecho central: **todas las salidas hacia un humano están enumeradas en `/smart`, y están en un solo nodo rojo**. Cada bloque tiene su zoom en la sección siguiente. Los colores importan; la leyenda está debajo.
...
P0{"<b>① /smart · paso 0</b><br/>¿ticket, prompt o retomar?"}:::dec
...
SHIP --> DW(["<b>⑦ deploy-watch.sh</b> · $0, solo CPU<br/>Actions → Kargo → ArgoCD health → rollout de los pods → smoke canary<br/>lo que NO pudo verificar se declara como supuesto"]):::script
...
Un ticket de Linear, un prompt literal entre comillas, o el `task-id` de una corrida que se murió. `/smart` decide cuál es sin preguntarte. El tercer caso es el que más vas a agradecer: como **todo el estado vive en `tasks/<id>/` y en los commits del worktree, nunca en la conversación de un agente**, una sesión muerta a mitad del pipeline se retoma con `/smart <task-id>`, y entra por la primera fase cuyo artefacto falte. Un artefacto válido jamás se re-genera.
...
|---|---|---|
| **quick** | ninguna: no se deduce, la declara el humano con `/quick`. Techos duros en `harness-policy.json`: **1 repo, 8 archivos, 200 líneas cambiadas** contra el merge-base | **toda la deliberación**: enrichment, abogados, RFC, DAG, briefs y plan. Quedan worktree, implementer, reviewer independiente y QA |
...
Tres redes lo hacen seguro. En duda entre dos carriles, gana el mayor. La clasificación es una *propuesta* que `ship.sh` verifica: **`gate_lane`** compara el diff real contra el carril declarado en `state.json`, y un express que tocó un `.proto` o una migración no pasa. Y escalar es barato y está pre-aprobado: `harness-policy.py escalate` sube el carril por la escalera `quick → express → standard → full` y re-encauza donde el carril destino recupera la deliberación (`/rfc` si la declara; `quick → express` cae en `intake`), conservando el worktree. **Equivocarse de carril cuesta una re-entrada, jamás un ship sin la deliberación que tocaba.** Los gates, la compliance matrix, la evidencia, el canary y el rollback son idénticos en los cuatro carriles: el carril recorta deliberación, nunca verificación.

**`quick` es la entrada mínima, y `/smart` la clasifica solo.** Sin enrichment, sin abogados, sin RFC ni plan: worktree y a implementar, en una sola sesión. Lo que no se toca es la verificación: trailer, precheck, todos los gates de `ship.sh`, reviewer independiente y QA determinista. Y la promesa se mide: un diff de más de 8 archivos o 200 líneas cambiadas es `gate_lane` en rojo, con la remediación escrita (`harness-policy.py escalate --to express`). La tarea no se pierde, sube de carril.

Durante un tiempo este fue el único carril que el harness **no** clasificaba, con el argumento de que lo que quick ahorra es justamente la deliberación que sirve para dimensionar, así que dimensionar era del humano. Lo dio vuelta un número: standard cuesta $119 por tarea contra $35 de express, y buena parte de lo que entraba por standard eran cambios mecánicos de un repo. Exigir que alguien escriba `/quick` para conseguir el carril barato significa que la tarea que nadie clasificó paga la ceremonia completa, y esa es la mayoría de las tareas.

Lo que sostiene esa clasificación automática son tres frenos y un límite declarado. `POLICY-LANE-005` rechaza el segundo repo en el acto (quick no genera DAG, así que nada ordenaría el ship). `POLICY-LANE-004` volvió a ser **rechazo duro** para quick: un repo de kind `infra-*` no entra, porque el piso de un carril que elige una máquina no puede depender de un juicio (en `express` y `triado` sigue siendo un aviso, que es lo correcto ahí). Y `gate_lane` frena contratos, migraciones e infra por patrón de archivo. **El límite, dicho en voz alta: ningún gate frena un cruce de ownership dentro de un mismo repo.** Un quick que escribe tablas de otro servicio pasa todo. Por eso clasificar quick exige evidencia positiva de que el cambio no escribe datos ajenos; sin esa evidencia, `express`. `/quick` sigue existiendo como entrada del humano que ya dimensionó: lo que cambió es que dejó de ser la única.
...
La parada por **abogado en `DRAFT`** parece burocracia y es lo contrario: la constitución de un abogado la propuso la arqueología leyendo tu código, pero hasta que un humano la ratifica **nadie la firmó**. Litigar citando una ley sin firmar es teatro. Por eso `/smart` para ahí, y es la primera cosa que vas a ratificar después de instalar.
...
**Y hay un segundo reloj, el de la SESIÓN que orquesta.** El watchdog vigila subagentes; a la sesión que los lanza no la vigilaba nadie, y ahí es donde vive el wall-clock. Medido sobre una tarea de un repo con una migración: 45.4 h de reloj, de las cuales 39.6 h fueron el orquestador solo (688 turnos, 440k de contexto medio, 758k de pico) contra 5.8 h de trabajo real de subagentes. Dos mecanismos lo atacan, los dos en `orchestrator-watch.sh`. El primero es el hueco de bus: más de 12 minutos sin un solo evento y sin llamada en vuelo significa sesión muerta, y se relanza sola con un lease atómico. El segundo es el **relevo de fase**: cuando `transition` avanza de verdad, deja `tasks/<id>/handoff.json`, el orquestador cierra su turno y el vigilante arranca una sesión nueva con contexto fresco. Tiene que ejecutarlo algo de afuera, porque un prompt no puede terminarse a sí mismo ni relanzarse. Las dos señales son ortogonales: una caza al agente que murió callado, la otra a la sesión que sigue viva arrastrando un contexto que ya no usa. Y la espera del deploy quedó fuera del camino caliente: son hasta 2820 s por ship que ahora los espera un proceso de bash a cero tokens, no una sesión con su ventana cargada. Los marcadores de relevo **caducan** (`HARNESS_ORCH_HANDOFF_TTL`, 6 h por defecto): un marcador huérfano de una tarea vieja no es un relevo pendiente, y sin caducidad se acumulan hasta que arrancar el vigilante dispara una sesión por cada uno. El vigilante además tiene **techo**, y no lo tenía: sus dos límites (el lease y los reintentos) eran por tarea, así que con un backlog de tareas viejas que cruzan el umbral de silencio a la vez no relanza una sesión, relanza decenas. Medido: 125 sesiones en 71 minutos, 29 en una sola pasada, a 1,15 GB cada una con su flota MCP, o sea 140 GB pedidos contra 12 de RAM, y la máquina hubo que reiniciarla a mano. Ahora hay cuatro frenos y cada uno cubre lo que los otros no: un tope de sesiones **vivas** (`HARNESS_ORCH_MAX_LIVE`, se cuentan los leases con PID vivo, que es dato que el lease ya guardaba), un tope **por pasada** para que suba en rampa y no en escalón (`HARNESS_ORCH_MAX_PER_PASS`), un **piso de memoria libre** que es el fail-safe de verdad cuando el tope está mal calibrado para la máquina (`HARNESS_ORCH_MIN_FREE_MB`), y un **techo de antigüedad** (`HARNESS_ORCH_MAX_AGE_H`): una tarea que lleva días en la misma fase no es una sesión que murió hace un rato, es una tarea abandonada, y se escala a humano por el camino que ya existe, que además la saca de la cola. Cuando el tope muerde, el orden es justo (menos intentos primero, desempate por más tiempo callada) para que la cola de atrás avance. Y el lease dejó de creerle a un PID pelado: guarda también el instante de arranque del proceso, porque después de un reinicio los PIDs se reciclan y un lease viejo daba "esta tarea tiene dueño" para una sesión que no existe. Medido: 19 marcadores viejos bastaron para que nadie se animara a levantarlo, y el pipeline entero corrió en una sola sesión con el contexto subiendo de 230k a 357k, o sea el relevo por fase desactivado de hecho por la basura que se junta sola.
...
```mermaid
flowchart TD
  A["cualquier agente<br/>implementer · reviewer · qa · /smart"]:::agent -->|"git push origin main"| H1{{"🚫 <b>block-direct-push</b><br/>hook PreToolUse · <b>fail-closed</b><br/>sin jq → bloquea por precaución"}}:::hook
...
Y hay una pregunta que ninguna de esas señales contesta: **¿los pods corren TU artefacto?** `Synced + Healthy` de ArgoCD dice que el cluster coincide con el *manifiesto* de esa revisión, y con promoción de imagen (Kargo) el tag lo bumpea un commit aparte, así que las dos cosas se separan solas. Medido en campo: 🟢 declarado con `updated=2 ready=3 available=2` y un pod viejo sirviendo un tercio del tráfico, o sea la guarda que el cambio agregaba "activa" mientras un pod seguía aceptando lo que la guarda rechaza. Por eso, cuando hay `kubectl`, el verde de cluster cruza dos cosas y no una: que el rollout **terminó** (`observedGeneration` al día, `updated == ready == available == replicas`, un solo juego de imagen entre los pods) y que los pods son **posteriores a tu commit**. Un contador de réplicas contesta "el rollout que hubiera terminó", no "mi cambio está sirviendo". Si el rollout no termina dentro de `DEPLOY_ROLLOUT_TIMEOUT`, no es rojo (nada está enfermo, está incompleto) y tampoco es verde: el tramo de cluster queda declarado ausente. Si no se puede mirar (sin `kubectl`, sin RBAC sobre el Deployment, o el Deployment vive en otra Application y no se declaró `DEPLOY_K8S_WORKLOADS`), es ceguera declarada y el verde se degrada al del manifiesto. Las otras dos señales aprendieron lo mismo. **Actions elige el run por quién construye, no por quién termina primero**: un push dispara varios workflows con el mismo sha, y tomar el primero de la lista hizo que el watcher cantara verde mirando el `e2e` con el `Deploy` todavía en vuelo, 7m41s antes de que la imagen existiera. Con un solo run no hay ambigüedad que resolver; con varios, se eligen los que tienen un job crítico (la misma declaración `critical_jobs` que ya existe), se puede fijar el workflow a mano (`DEPLOY_WORKFLOW` o `deploy: <repo>: workflow:`), y si ninguno es reconocible se declara ceguera en vez de adivinar. **Y una lista de promociones vacía de Kargo dejó de leerse como un aviso**: si no hay freight posterior a tu ship, el artefacto todavía no llegó, y eso es un tramo sin verificar, no un tramo verde. El smoke, que es la última defensa, no corre cuando el rollout quedó incompleto: interrogaría a los pods de la versión anterior y saldría verde sin decir nada de tu cambio.
...
**Un job saltado del pipeline se mide por lo que hace, no por el hecho de saltarse.** Un `build` en `skipped` deja el run en `success` y producción con el binario viejo: eso es rojo, y por eso el watcher mira la conclusión de cada job y no solo la del run. Pero tratar a **todos** los jobs por igual convertía un `docs-dry-run` condicional en el mismo rojo, y un verificador que grita lobo se ignora justo el día que acierta. Los jobs que producen o despliegan el artefacto son un eje declarado: `critical_jobs` por repo en `harness-answers.yaml`, `DEPLOY_CRITICAL_JOBS` para una corrida suelta, y `build` por default (se compara por token, así que cubre `build-and-push` y no `rebuild-cache`). Un skipped no crítico se dice y no frena; si **ningún** job del run se reconoce como crítico, el watcher declara ceguera y dice cómo declararlo, en vez de inventar un verde o un rojo.
...
Cuando el canary queda verde, `/archive` **fusiona el delta-spec en la spec maestra automáticamente**. Esta es la razón por la que el SDD de este harness no muere: si esa fusión dependiera de la disciplina humana, en un trimestre las specs mentirían, y una spec podrida es peor que ninguna, porque los agentes la ejecutan con confianza. Lo que no se fusiona es un delta que ningún reviewer vio: `POLICY-ARCHIVE-001` compara el **hash** del `delta-spec.md` de ahora contra el que cada veredicto declara haber revisado (`delta_spec_sha256`). Antes comparaba mtimes y eso lo derrotaba el propio flujo, porque fundir el campo `qa` en el veredicto es un paso posterior al juicio del reviewer: bastaron 17 segundos de ventana para que una enmienda del delta pasara en verde.
...
`/smart` corre solo, pero "solo" no debería significar "a ciegas". `make ui` abre el panel local (por defecto `127.0.0.1:7180`) que te deja ver, mientras el harness trabaja: **qué agentes están vivos ahora mismo** y en paralelo, en qué fase va cada tarea, el texto que van produciendo, tokens y costo por agente, el grafo de quién lanzó a quién, y **el ledger de supuestos** de cada tarea.
...
El formulario de Nueva tarea escribe preferencias que `/smart` **respeta como ley**: `review_before_ship: true` fuerza una pausa antes del primer ship, `assumptions_ok: false` convierte cada ambigüedad en una parada en vez de un supuesto, `max_parallel` acota los implementers y `budget_usd` convierte pasarse de presupuesto en una parada.
...
| **Graphify** (CLI) | Knowledge graph del código cross-repo (Tree-sitter y detección de comunidades). | Las preguntas de *comprensión* ("¿quién consume este servicio?", "¿qué camino conecta A con B?") se responden con el grafo (~71 veces menos tokens, [cifra reportada por Graphify](https://github.com/Graphify-Labs/graphify)) en vez de con búsquedas masivas. Lo usan arquitecto y orquestador; los implementers no lo necesitan porque Serena cubre el nivel de símbolo. **El grafo se mantiene solo**: `graph-refresh.sh` corre en el prefetch de `/smart`, en harness-janitor y en `make graph`; el doctor avisa si graphify está instalado sin grafo construido. |
...
| **harness-cost.py** | La báscula: costo real por agente, por rol y por tarea, leyendo los transcripts. Cero tokens de modelo. | `task <id>` da el desglose, `day` los totales, y `check <id>` es el **gate** que `transition` corre solo: sale 3 si el gasto excede el presupuesto, si el contexto medio pasa el techo o si el acierto de caché cae del piso. Medido sobre 663 transcripts: el 87% del gasto vive en la sesión orquestadora y el top 10% de las sesiones se lleva el 66%, así que lo que faltaba no era un precio más bajo sino un disyuntor para la cola. **Cada término tiene su propia salida auditable**: `budget --to` mueve el techo en dólares (y solo el término del presupuesto), y `harness-policy.py cost-waive --band cache\|ctx --agent <rol>` acepta con actor y motivo lo que ya no se puede remediar, porque sale de transcripts de un agente cerrado. El piso de caché no se le cobra a un agente demasiado corto: con T turnos el máximo alcanzable es `(T-1)/T`, así que por debajo de `1/(1-piso)` turnos el umbral mediría la ventana de caché y no el derroche. **Y los dos términos de tasa miden la FASE EN CURSO**, no toda la historia: son promedios sobre transcripts inmutables, así que sin ventana el primer agente que cierra bajo el piso congela la tarea para siempre (dos abogados de RFC de una sola respuesta trabaron una tarea que globalmente estaba en 94.4%). `transition` estampa `phase_since` y el check mide desde ahí; lo que queda fuera de la ventana se declara en cada corrida. **Y la tarea de cada transcript sale de las RUTAS que nombra** (`worktrees/<id>/`, `tasks/<id>/`), no del puntero por sesión que escribe el hook: ese puntero es uno por sesión y el último gana, así que en un barrido donde un orquestador lanza varias tareas, una se llevaba todo el gasto y las hermanas quedaban en cero (medido: una tarea express de 12 líneas de diff con 477 turnos de orquestador y ocho implementers ajenos, mientras las cuatro que hicieron el trabajo real reportaban "sin transcripts"). Como las rutas son dato inmutable, la atribución además sobrevive al cierre de la sesión, que es cuando se archiva. El reporte marca también el agente `general-purpose`, que está prohibido en el pipeline.  **Y desde ahora reporta TIEMPO**, en dos columnas que no son la misma pregunta: `activo` suma solo los huecos entre turnos que son trabajo, y `reloj` va del primer turno al último del agente, con el ocio adentro. La diferencia es el dato: la duración que el runtime informa es inconsistente para los agentes CONTINUADOS, que es justo como el harness hace las rondas, y con ese número una ronda de 12 minutos parecía de 34 y el diagnóstico de latencia apuntaba al lugar equivocado. |
| **finding.sh** | El canal de difusión entre repos hermanos de una misma tarea. | `publish` deduplica y redacta secretos; `read` devuelve una lista **acotada** (entra en la ventana de un agente). Existe porque tres repos escribieron el mismo guard roto por separado y el único difusor era el humano. |
| **task-note.py** | La nota que sobrevive a la tarea: `docs/tareas/<id>.md`, versionado. | `tasks/` esta gitignoreado, asi que el ledger, los veredictos y `history[]` mueren con la maquina, y con N ingenieros son N maquinas sin aprendizaje compartido. El script rellena lo verificable (costo real, rondas, supuestos separando lo medido de lo asumido, tardios) y deja marcado lo de juicio. Los `[[enlaces]]` hacen de `docs/` un vault de Obsidian sin infraestructura nueva. **No guarda el proceso**: una traza de razonamiento decae en dias, lo que compone es la sorpresa. |
| **harness-sink.py** | El destino de los datos: `docs/metrics/<tarea>.jsonl` siempre, Postgres opcional. `setup` pregunta y **prueba la conexion**. | Una fila por AGENTE con costo, tokens, contexto, acierto de cache, carril, rondas y el link al ticket. Empezar por el archivo no cierra la puerta a la base; empezar por la base si cierra la puerta a la simplicidad: `duckdb -c "SELECT carril, avg(costo_usd) FROM read_json_auto('docs/metrics/*.jsonl') GROUP BY 1"` es toda tu empresa consultable con cero infraestructura. |
| **repo-brief.sh** | Resumen determinista por repo (`.cache/briefs/<repo>.md`, cacheado por HEAD): stack, comandos de test, estructura, convenciones, y los **homónimos** del repo (identificadores definidos también en otro repo, leídos del grafo). | El arranque en frío de cada implementer o reviewer re-descubría lo mismo en cada tarea, y eso son minutos y miles de tokens de exploración. El resumen se genera una vez con $0 tokens y viaja en el prompt: el agente arranca editando, no explorando. Los homónimos van acá y no en un prompt por una razón medida: pedirle al agente que consulte el grafo cuesta 2 o 3 tool calls a contexto completo, y el grep que ya tiene a mano contesta algo en una, así que gana el grep. La respuesta viaja resuelta. |
| **Carriles** (quick, express, standard, full) | El pipeline dimensionado al blast radius (ver ②b). | El ahorro más grande de todos: una tarea chica pasa de unas 6 sesiones LLM a 2, y en `quick` no hay ninguna fase de deliberación. Menos sesiones son menos arranques en frío, o sea menos tokens **y** menos minutos, con los mismos gates. |
...
**Y estas herramientas reemplazan AGENTES, que es donde estaba el dinero.** Una pregunta sobre el código (dónde está X, quién llama a Y, qué hace Z) no es trabajo para un subagente: es una consulta. Un agente cuesta su contexto de arranque más todos sus turnos; una consulta cuesta una tool call. Medido en UNA tarea: 34 subagentes `general-purpose` gastaron 3134 turnos y $357, el rubro más caro de sus $953 totales, mientras el único `Explore` de la misma tarea resolvió lo suyo en 33 turnos y $2. Ningún prompt del harness lo nombraba nunca: se usaba porque es el default del tool Agent y nada decía otra cosa. Hoy `general-purpose` está **prohibido** en el pipeline, `Explore` es el agente para cuando de verdad hace falta uno, y `harness-cost.py` marca la fila si aparece, así que el criterio se verifica solo. Por el mismo motivo los agentes se **continúan** en vez de relanzarse: los ids viven en `tasks/<id>/agents.json` y la ronda siguiente es un mensaje al mismo agente, porque uno nuevo vuelve a pagar el contexto que el anterior ya tenía (medido: 25 implementers y 16 reviewers donde correspondían del orden de 2 y 2).
...
```
/smart COR-123 --model deep       # ← una sola tarea, sin tocar nada
```
...
- **`ship.sh`**: la única puerta a main. En serie lo que tiene orden real: rebase, trailer `Task:`, **carril** (`gate_lane`, el diff respeta lo que el intake declaró). Después, **en paralelo**: build y test por lenguaje más `buf breaking`, `gitleaks` y `semgrep`, **tests no debilitados**, y veredicto con compliance, **evidencia** y policy. Los gates de lenguaje cubren Go (raíz y subdirectorios), Node/TS, Python, Dart, Rust, Java, Ruby, PHP, .NET, Elixir, Terraform (`init -backend=false` + `validate` + `fmt -check`) y Helm (`lint` + `template`, y `unittest` si el plugin está; nunca `dependency update`, que reescribiría el `Chart.lock` del árbol que el gate juzga), y si no reconocen el stack **lo dicen**: un gate que no compiló ni testeó nada no puede salir en verde callado. Al final: lock por repo y push. Los rojos se reportan juntos y **el error de cada gate es un prompt**: incluye su remediación, para que el agente corrija todo en una iteración (máximo 2 rondas de autofix).
- **Gates activados por config**: la configuración del repo es el opt-in; con ella presente son gates duros, sin ella hay silencio. Son `import-linter` (fronteras de capas en Python, `.importlinter`), `go-arch-lint` (grafo de dependencias en Go, `.go-arch-lint.yml`) y `squawk` (lint de migraciones SQL nuevas del diff; las viejas no se re-litigan). Config presente sin herramienta instalada es un aviso honesto, jamás un gate fingido.
  ...
  Los tres gates de integridad existen porque nos hicieron una pregunta incómoda: *nuestros gates confían en cosas que el agente puede editar.*

  - **`gate_tests_untouched`**: el gate de test confía en la suite de tests, y la suite es un archivo. La forma más barata de pasar a verde no es arreglar el código: es borrar la aserción. Está medido en la literatura (en [SWE-Bench+](https://arxiv.org/abs/2410.06992), cerca del 31% de los parches "exitosos" pasaban gracias a tests débiles) y los harnesses que solo escriben *"no borres tests"* en prosa lo escriben porque no tienen gate. Este bloquea aserciones eliminadas, `skip`s añadidos y tests borrados, **salvo** que el delta-spec declare el cambio como `MODIFIED` o `REMOVED`, porque entonces no es trampa: es cumplir la spec. Los tests son el contrato; cambiar uno es un RFC. La otra excepción no necesita declaración y es de otra naturaleza: los `skip`s de un archivo de test **AÑADIDO** no se cuentan, porque un archivo que no existía no puede debilitar nada, y el guard de entorno que los propios runners piden (`test.skip(!reachable, …)`, `t.Skip`) hacía que todo spec nuevo saliera rojo sin salida legítima. El archivo igual se nombra en la salida, y el residual (un spec nuevo enteramente skipeado) queda a cargo de `gate_test_muerde` con la cobertura y los huecos que su propio comentario documenta.
  ...
  - **`gate_test_muerde`**: los dos anteriores vigilan los tests que existen; nada vigilaba los que llegan. La forma más barata de "cubrir" un requisito no es borrar una aserción: es escribir una que no puede fallar (caso de campo: un assert que evaluaba antes de que el dato llegara pagó una ronda entera, con commit, precheck, dos sellos de evidencia y dos agentes, sin probar nada). En el precheck, todo test NUEVO del diff (y todo el que el delta-spec nombre bajo `ADDED`) se corre también sobre el árbol BASE con tus tests encima: si pasa ahí, no prueba la conducta nueva, y el rojo trae su remediación (escribí el test para que falle primero; si es refactor de test, declaralo `MODIFIED` y sale del alcance). Los tests modificados sin nombrar quedan fuera a propósito: ahí vivirían los falsos rojos. Donde no hay runner dirigido inequívoco, el gate lo declara como supuesto en vez de fingir un veredicto. Y cuenta lo que de verdad corrió: si TODO el alcance quedó sin mirar (la base no resuelve sus módulos, ningún runner colecta el archivo), el gate lo dice con esas palabras en vez de cerrar con un ✅, que era el mismo mensaje que ponía cuando sí había medido. Un vacuo y uno bueno recibían la misma respuesta.
...
| Modelo para una tarea puntual | `/smart <id> --model deep` (o `model:` en el frontmatter de `task.md`) |
...
Por eso el repo publica **`templates/MANIFEST.sha256`**: el sha256 de cada uno de los 116 archivos que terminan dentro de una instancia, más un `digest:` que identifica al set completo. Y todo generador **debe** escribir ese digest en `.harness-templates` al generar. (Ese 116 también lo verifica la suite contra el manifiesto real: ver el final de esta sección.)
...
Y hay un tercer eje que el número y el digest no cubren: **el generador**. `/harness-update` prefiere el binario `harness` del tap sobre la re-instanciación manual, con razón, porque es determinista; pero ese binario trae su propia copia de los templates y su propio número. En campo llegó a reportar `0.60.0` con el último tag de upstream en `0.59.3`: una versión que no existe en ningún origen. Generar con él habría escrito templates no publicados y estampado ese número en `.harness-version`, y el `--verify` posterior habría salido rojo sin explicar por qué. Por eso `scripts/harness-version.sh --generator` lo pregunta antes de que se escriba nada, con contrato por exit code: **0** su versión es el último tag publicado, **1** no lo es (no existe, es un pre-release, o es un tag viejo), **2** no se pudo comprobar. Exit 2 no autoriza: es una comprobación que no corrió. Antes esto eran tres párrafos de prosa pidiéndole al humano que comparara a mano, y el incidente `0.60.0` ocurrió con la prosa ya escrita.
