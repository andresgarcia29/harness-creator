...
| `.claude/settings.json` | settings.json.tmpl | siempre (hooks + `permissions` + bloque `env`). El `permissions` son TRES piezas y las tres hacen falta: `deny` (destructivo prohibido), `allow` (las CUATRO formas de invocar lo del harness sobre `scripts/*` (la pelada y los tres lanzadores) más `git`) y `defaultMode: acceptEdits`. Sin las dos ultimas, la sesion HEADLESS que `orchestrator-watch.sh` levanta por fase (`claude -p "/smart <id>"`) no tiene a quien pedirle aprobacion: cada tool call queda denegada en silencio y el relevo consume el handoff sin poder escribir un byte, ni siquiera para registrar la pausa (COR-1446: cuatro relevos con cero bytes, 146k tokens de architect no materializables, horas varadas sin registro). Lo que protege la escritura no es el modo de permiso sino los hooks de Edit|Write (`guard-canonical`, `guard-worktree`), que corren igual. La forma de las reglas importa: `Bash(bash scripts/*)` concede y `Bash(bash scripts/:*)` no (el prefijo se corta en frontera de token), verificado contra `claude -p`. Y el patrón matchea el comando ENTERO desde el principio, así que `Bash(scripts/*)` (la forma PELADA, sin intérprete) no es un lujo: es la forma en la que /smart y CLAUDE.md escriben cada invocación (`scripts/ship.sh ...`, `scripts/harness-policy.py transition ...`), y sin esa regla no matchea ninguna. El modo de falla no es un prompt de más: el relevo headless recibe "This command requires approval", concluye que nació amordazado, lo escribe en el handoff y cierra el turno sin mover la fase (caso de campo: dos relevos consecutivos perdidos y la fase `deploy` sin cerrar ~1h sobre un deploy ya verde, mientras `python3 scripts/harness-policy.py` corría en la misma sesión sin una sola aprobación). El agente no puede descubrirlo solo: el rebote nombra el comando denegado, nunca la regla que lo habría admitido. Las cuatro reglas van juntas y no se reescriben los prompts a la forma con prefijo: eso es mucha superficie y vuelve a driftear. Y el `allow` de un settings de PROYECTO lo DESCARTA el CLI hasta que el workspace esté confiado (`Ignoring N permissions.allow entries: this workspace has not been trusted`), así que abrir Claude Code una vez interactivo acá y aceptar el diálogo es parte de instalar; `defaultMode` sí aplica sin confianza. Al RELEVO eso no lo alcanza porque corre donde nadie ve ese aviso: `orchestrator-watch.sh` lee este mismo `allow` y se lo pasa por `--allowedTools`, que sí concede sin confianza. O sea que este bloque es la fuente única de las dos rutas, y un `allow` vacío deja al relevo sin nada. El `env` no es decorado: lo que se declara ahí llega al ENTORNO DE LOS HOOKS y al de todo proceso que la sesión lance, incluido el `claude -p` que `orchestrator-watch.sh` levanta por tarea. De ahí cuelga `PONYTAIL_DEFAULT_MODE`, la perilla del plugin ponytail (`off\|lite\|full\|ultra`), fijada en `lite`. Es una elección de COMPORTAMIENTO y no de costo, y la distinción se midió corriendo el hook: `lite`, `full` y `ultra` inyectan el mismo ruleset salvo por unos 6 tokens (~1400 cada uno), y la única palanca que ahorra es `off`, que inyecta 2 bytes. O sea que bajar de `full` a `lite` para "gastar menos" no gasta menos: si lo que se busca es recortar los ~1400 tokens que se pagan por CADA nodo del DAG, el valor tiene que ser `off`. La var es inerte para quien no tenga el plugin instalado |
