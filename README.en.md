...
In multi-repo tasks there is a trap the harness now closes: `ship.sh` runs once per repo and requires the `review` phase, so advancing the phase before the last repo had published left the remaining ones with no way back. `POLICY-SHIP-004` rejects that advance while any repo still has a verdict and no entry in `ship-ledger.jsonl` (the phase ledger: one JSON line per published repo; it used to be called `ship.log`, a name that invited redirecting the ship's stdout over it and truncating it), and it tells you which ones are missing.
