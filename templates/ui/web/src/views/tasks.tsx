...
import { BUSKINDS, pending, taskRollup, sessionsOfTask, rollupSessions, fecha, toEpoch,
  type Snapshot, type Task } from "@/lib/harness"
...
export function Tasks({ s, go }: { s: Snapshot; go: Go }) {
  ...
  const roll = taskRollup(s)
  // más reciente arriba: la lista es un pulso, no un archivo
    ...
    <>
      ...
      <div className="grid gap-2.5">
        {sorted.map((id) => {
          ...
          const r = roll.find((x) => x.id === id)
          // agentes vivos AHORA en sus sesiones (vínculo por cwd/runs/bus)
          const live = rollupSessions(sessionsOfTask(s, id, tevs)).live
          const done = r?.status === "ship" && r.phase === "archive"
          return (
            <div key={id} role="button" tabIndex={0} onClick={() => go({ name: "task", id })}
              ...
              {live > 0 && (
                <span className="flex shrink-0 items-center gap-1.5 text-[11px] font-semibold text-(--ok)">
                  <i className="size-1.5 animate-pulse rounded-full bg-(--ok)" />{live} vivo{live > 1 ? "s" : ""}
                </span>
              )}
              {p && <StatusBadge on={false}>{p._k === "block" ? "✕ bloqueada" : "⏸ te espera"}</StatusBadge>}
              {!p && done && <StatusBadge on>✓ terminada</StatusBadge>}
              {!p && !done && r?.status === "ship" && <StatusBadge on>▲ en main</StatusBadge>}
              ...
              <NumCell v={done ? "✓" : t.phase || r?.phase || "—"} l="fase" className="hidden md:block" />
              <NumCell v={t.assumptions.length} l="supuestos" className="hidden md:block" />
              <NumCell v={lastTs ? fecha(lastTs) : "—"} l="últ. evento" />
