...
3. Clasifica cada diferencia según la PROPIEDAD del archivo:
   - **Propiedad del plugin**: TODO `scripts/` sale del plugin, así que la
     ...
     ticket-close,ticket-pull,linear,jira,harness-bug,harness-version,skills-sync,instance-repo,
     ...
     **los hooks Y el archivo que los cablea**, que van JUNTOS o el update
     ...
     guard-build-slot,guard-gcloud,guard-ws-scripts,guard-broad-add,guard-symbol-grep,
     track-read,ui-emit,on-compact,mem-recall,session-summary}.sh` más
